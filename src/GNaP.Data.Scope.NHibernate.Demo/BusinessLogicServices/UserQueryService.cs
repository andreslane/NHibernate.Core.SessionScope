namespace GNaP.Data.Scope.NHibernate.Demo.BusinessLogicServices
{
	...
	{
        private readonly NHibernateScopeFactory sessionFactory;
		private readonly IUserRepository _userRepository;

        public UserQueryService(NHibernateScopeFactory sessionFactory, IUserRepository userRepository)
		{
		...
		public User GetUser(Guid userId)
		{
			 ...
			 */
			using (var scope = sessionFactory.CreateReadOnly())
			{
                ISession session = scope.GetFromFactory<UserSessionFactory>();
		...
		{
            using (var session = sessionFactory.CreateReadOnly()) 
            {
                ISession realSession = session.GetFromFactory<UserSessionFactory>();
                return realSession.CreateCriteria<User>()
		...
		public User GetUserViaRepository(Guid userId)
		{
			 ...
			 */
		    using (sessionFactory.CreateReadOnly())
		    {
	    ...
	    public User GetUserUncommitted(Guid userId)
		{
			 ...
			 */
            using (var session = sessionFactory.CreateReadOnlyWithTransaction(IsolationLevel.ReadUncommitted))
            {
