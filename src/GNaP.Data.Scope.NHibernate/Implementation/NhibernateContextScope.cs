namespace GNaP.Data.Scope.NHibernate.Implementation
{
    ...
    public class NHibernateContextScope : IDbContextScope
    {
        private bool _disposed;
        private bool _completed;
        private readonly bool _readOnly;
        private readonly ISessionFactory _sessionFactory;
        private readonly NHibernateContextScope _parentScope;
        private readonly bool _nested;
        private readonly NHibernateContextCollection _contexts;

        public NHibernateContextScope(ISessionFactory sessionFactory = null) :
            this(joiningOption: DbContextScopeOption.JoinExisting, readOnly: false, isolationLevel: null, sessionFactory: sessionFactory)
        { }

        public NHibernateContextScope(bool readOnly, ISessionFactory sessionFactory = null)
            : this(joiningOption: DbContextScopeOption.JoinExisting, readOnly: readOnly, isolationLevel: null, sessionFactory: sessionFactory)
        { }

        public NHibernateContextScope(DbContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel, ISessionFactory sessionFactory = null)
        {
            ...
            {
                _nested = false;
                _contexts = new NHibernateContextCollection(readOnly, isolationLevel);
            }
        ...
        private static readonly ConditionalWeakTable<InstanceIdentifier, NHibernateContextScope> DbContextScopeInstances = new ConditionalWeakTable<InstanceIdentifier, NHibernateContextScope>();
        ...
        /// </summary>
        internal static void SetAmbientScope(NHibernateContextScope newAmbientScope)
        {
        ...
        /// </summary>
        internal static NHibernateContextScope GetAmbientScope()
        {
            ...
            NHibernateContextScope ambientScope;
...
" + Environment.StackTrace;
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            foreach (var initializedContext in _contexts.InitializedDbContexts)
            {
                var sessionInCurrentScope = initializedContext.Value;
                var correspondingParentContext =
                    _parentScope._contexts.InitializedDbContexts.SingleOrDefault(parentContext => parentContext.Key == initializedContext.Key).Value;
        ...
        private static void RefreshEntity(ISession correspondingParentContext, ISession sessionInCurrentScope, object toRefresh)
        {
            ...
            if (parentContext.EntitiesByKey.TryGetValue(entityInThisContext.Key, out objectInParentContext))
            {
                ...
                var currentState = entityInParentContext.Persister.GetPropertyValues(objectInParentContext,
                    parentSessionImplementation.EntityMode);
                if (entityInParentContext.Persister.FindDirty(currentState, entityInParentContext.LoadedState,
                    entityInParentContext, parentSessionImplementation) == null)
                {
        ...
        {
            return _contexts.Get<TDbContext>();
        }

        public TDbContext Get<TDbContext, TDbContextFactory>() where TDbContext : ISession
        {
            return _contexts.Get<TDbContext, TDbContextFactory>();
        }

        public ISession GetFromFactory<TDbContextFactory>()
        {
            return _contexts.GetFromFactory<TDbContextFactory>();
        }
    }
