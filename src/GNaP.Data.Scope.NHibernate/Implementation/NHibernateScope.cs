    ...
    internal class NHibernateScope : IDbScope
    {
        ...
        public NHibernateScope(DbScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel)
        {
            ...
            if (_parentScope != null && joiningOption == DbScopeOption.JoinExisting)
            {
                ...
                {
                    throw new InvalidOperationException("Cannot nest a read/write NHibernateScope within a read-only NHibernateScope.");
                }

                _nested = true;
                _sessions = _parentScope._sessions;
            }
            else
            {
                _nested = false;
                _sessions = new NHibernateSessionCollection(readOnly, isolationLevel);
            }

            SetAmbientScope(this);
        }

        public ISession Get<TDbContextFactory>()
            where TDbContextFactory : IDbFactory<ISessionFactory>
        {
            return _sessions.GetFromFactory<TDbContextFactory>();
        }

        public int SaveChanges()
        {
            if (_disposed)
                throw new ObjectDisposedException("NHibernateScope");

            if (_completed)
                throw new InvalidOperationException("You cannot call SaveChanges() more than once on an NHibernateScope. An NHibernateScope is meant to encapsulate a business transaction: create the scope at the start of the business transaction and then call SaveChanges() at the end. Calling SaveChanges() mid-way through a business transaction doesn't make sense and most likely mean that you should refactor your service method into two separate service method that each create their own NHibernateScope and each implement a single business transaction.");
        ...
        private static void RefreshEntity(ISession correspondingParentContext, ISession sessionInCurrentScope, object toRefresh)
        {
            var parentSessionImplementation = correspondingParentContext.GetSessionImplementation();
            var parentContext = parentSessionImplementation.PersistenceContext;
            var thisContext = sessionInCurrentScope.GetSessionImplementation().PersistenceContext;

            if (thisContext.EntitiesByKey.All(pair => pair.Value != toRefresh))
            {
                return;
            }

            var entityInThisContext = thisContext.EntitiesByKey.SingleOrDefault(pair => pair.Value == toRefresh);
            object objectInParentContext;
            if (parentContext.EntitiesByKey.TryGetValue(entityInThisContext.Key, out objectInParentContext))
            {
                var entityInParentContext = (EntityEntry)parentContext.EntityEntries[objectInParentContext];
                var currentState = entityInParentContext.Persister.GetPropertyValues(objectInParentContext, parentSessionImplementation.EntityMode);

                if (entityInParentContext.Persister.FindDirty(currentState, entityInParentContext.LoadedState, entityInParentContext, parentSessionImplementation) == null)
                {
                    correspondingParentContext.Refresh(objectInParentContext);
                }
            }
        }
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            foreach (var initializedContext in _sessions.InitializedDbContexts)
            {
                var sessionInCurrentScope = initializedContext.Value;
                var correspondingParentContext = _parentScope._sessions
                                                             .InitializedDbContexts
                                                             .SingleOrDefault(parentContext => parentContext.Key == initializedContext.Key)
                                                             .Value;
                ...
                {
                    RefreshEntity(correspondingParentContext, sessionInCurrentScope, toRefresh);
                }
            }
        }
        ...
        public void Dispose()
        {
            ...
            if (currentAmbientScope != this) // This is a serious programming error. Worth throwing here.
                throw new InvalidOperationException("NHibernateScope instances must be disposed of in the order in which they were created!");
            ...
            if (_parentScope != null)
            {
                if (_parentScope._disposed)
                {
                    ...
                    var message = @"PROGRAMMING ERROR - When attempting to dispose an NHibernateScope, we found that our parent NHibernateScope has already been disposed! This means that someone started a parallel flow of execution (e.g. created a TPL task, created a thread or enqueued a work item on the ThreadPool) within the context of an NHibernateScope without suppressing the ambient context first. 
...
" + Environment.StackTrace;
        ...
        private static readonly string AmbientDbContextScopeKey = "AmbientNHibernateDbContext_" + Guid.NewGuid();
        ...
        private static readonly ConditionalWeakTable<InstanceIdentifier, NHibernateScope> DbContextScopeInstances = new ConditionalWeakTable<InstanceIdentifier, NHibernateScope>();
        ...
        /// </summary>
        internal static void SetAmbientScope(NHibernateScope newAmbientScope)
        {
        ...
        /// </summary>
        internal static NHibernateScope GetAmbientScope()
        {
            ...
            Debug.WriteLine("Programming error detected. Found a reference to an ambient NHibernateScope in the CallContext but didn't have an instance for it in our DbContextScopeInstances table. This most likely means that this NHibernateScope instance wasn't disposed of properly. NHibernateScope instance must always be disposed. Review the code for any NHibernateScope instance used outside of a 'using' block and fix it so that all NHibernateScope instances are disposed of.");
            return null;
        }
